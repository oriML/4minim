...
export default function CustomSetPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      console.log("CustomSetPage useEffect: Starting fetchData");
      const products = await getProductsByCategory();
      setProductsByCategory(products);
      console.log("CustomSetPage useEffect: productsByCategory loaded", products);

      const storedProductsJson = localStorage.getItem('preselectedSetProducts');
      console.log("CustomSetPage useEffect: storedProductsJson from localStorage", storedProductsJson);

      if (storedProductsJson) {
        try {
          const preselectedProductsMap: Record<string, { qty: number }> = JSON.parse(storedProductsJson);
          console.log("CustomSetPage useEffect: preselectedProductsMap", preselectedProductsMap);

          const allProducts = await getProductsAction(); // Fetch all products
          console.log("CustomSetPage useEffect: allProducts loaded", allProducts);
          ...
          console.log("CustomSetPage useEffect: preselected products array", preselected);
